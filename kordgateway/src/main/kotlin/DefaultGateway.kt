...
private val defaultGatewayLogger = LoggerFactory.getLogger(DefaultGatewayData::class.java)


internal fun Throwable.isTimeout() = this is UnresolvedAddressException
    ...
    private suspend fun sendUnsafe(command: Command) {
        ...
        if (command is Identify) {
            defaultGatewayLogger.trace(
                "Gateway >>> {}",
                Json.encodeToString(Command.SerializationStrategy, command.copy(token = "token"))
            )
...
internal val GatewayConfiguration.identify
    get() = Identify(
        token,
        OptionalLong.Value(16381L),
        IdentifyProperties(os = os, browser = name, device = name),
        false.optional(),
