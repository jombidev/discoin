...
private val jsonLogger = LoggerFactory.getLogger(Event::class.java)
...
//                    jsonLogger.info("ReadConfirm: {}", data)
//                    UnknownDispatchEvent(name, data, sequence)w
                    MessageReadConfirm(
                        data = decoder.decodeSerializableElement(
                            descriptor,
                            index,
                            MessageACKData.serializer()
                        ),
                        sequence
                    )
                }
                ...
                "RELATIONSHIP_REMOVE" -> RelationShipRemove(
                    data = decoder.decodeSerializableElement(
                        descriptor,
                        index,
                        RelationShipRemoveData.serializer()
                    ),
                    sequence
                )
...
public sealed class Close : Event() {
     ...
     */
    public data object Reconnecting : Close()

    /**
     * The gateway closed and will attempt to start a new session.
     */
    public data object SessionReset : Close()
     ...
     */
    public data object ZombieConnection : Close()
     ...
     */
    public data object RetryLimitReached : Close()
}

public data object HeartbeatACK : Event()
public data object Reconnect : Event()
...
public data class RelationShipAdd(
    val data: RelationShipData,
    override val sequence: Int?,
) : DispatchEvent()

public data class RelationShipRemove(
    val data: RelationShipRemoveData,
    override val sequence: Int?,
) : DispatchEvent()
...
public data class MessageReadConfirm(
...
@Serializable
public data class MessageACKData(
    val version: Int,
    @SerialName("message_id")
    val messageId: Snowflake,
    @SerialName("mention_count")
    val mentionCount: OptionalInt = OptionalInt.Missing,
    ...
    val channelId: Snowflake,
    @SerialName("ack_type")
    val ackType: OptionalInt = OptionalInt.Missing,
)
