...
private val logger = LoggerFactory.getLogger(IdentifyRateLimiter::class.java)
...
private class IdentifyRateLimiterImpl(
    ...
    private fun CoroutineScope.launchIdentifyWaiter(previousWaiter: Job?, request: IdentifyRequest) = launch {
        if (previousWaiter != null) {
            logger.debug(
                "Waiting for other shard(s) with rate_limit_key {} to identify " +
                        "before identifying on shard {}", request.rateLimitKey, request.shardId
            )
            previousWaiter.join()
        }
    ...
    private companion object {
        ...
        private val ExceptionLogger = CoroutineExceptionHandler { context, exception ->
            ...
            logger.error("IdentifyRateLimiter threw an exception in context $context, please report this, it should not happen", exception)
        }
    }
}
