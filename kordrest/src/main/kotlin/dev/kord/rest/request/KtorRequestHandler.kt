...
public class KtorRequestHandler(
    ...
    private val logger = LoggerFactory.getLogger("[R]:[KTOR]:[${requestRateLimiter::class.simpleName}]")
    ...
    override tailrec suspend fun <B : Any, R> handle(request: Request<B, R>): R {
        ...
        return when {
            ...
            response.isError -> {
                logger.error(response.logString(body))
    ...
    private suspend fun <B : Any, R> HttpClient.createRequest(request: Request<B, R>) = prepareRequest {
        ...
        when (request) {
            ...
            is MultipartRequest -> {
                ...
                logger.debug(
                    request.logString(content.filterIsInstance<PartData.FormItem>()
